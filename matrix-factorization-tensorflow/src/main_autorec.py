...
sparse_ds = tf.data.Dataset.from_tensor_slices((sparse_dataset, sparse_dataset)).batch(128)
...
model = AutoRecommender(m=n_items, k=300, reg=3e-3)
model.compile(loss=custom_sparse_mse, optimizer=optimizers.Adam(learning_rate=0.001))
print(model.summary())
model.fit(sparse_train, validation_data=sparse_test, epochs=50)
